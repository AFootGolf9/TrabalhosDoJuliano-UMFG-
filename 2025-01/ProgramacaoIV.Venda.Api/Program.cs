...
var connectionString = "Server=localhost;Port=5432;Database=dotnet;Uid=dotnet;Pwd=dotnet;";


var builder = WebApplication.CreateBuilder(args);

// builder.Services.AddDbContext<VendaContext>(options => options.UseMySQL(connectionString));
builder.Services.AddDbContext<VendaContext>(options => options.UseNpgsql(connectionString));
...
app.MapPost("/transacoes/{idTransacao}/itens", async (string idTransacao, [FromBody] TransacaoItemRequest request, VendaContext context) =>
{
    try
    {
        ...
        if (produto.Estoque < request.Quantidade)
            throw new InvalidOperationException($"Quantidade solicitada ({request.Quantidade}) excede o estoque dispon�vel ({produto.Estoque}) para o produto {produto.EAN} - {produto.Descricao}. Ajuste a quantidade ou verifique o estoque.");
